...
11. Jiufang FBA orders should send an explicit empty `Importer` object (`Address.AddressLine: [null]`) so the website's 进出口委托 section stays unfilled by default. Do not copy `ShipFrom` company name or credit code into `Importer` unless a reviewed customs-declaration workflow explicitly requires it.
