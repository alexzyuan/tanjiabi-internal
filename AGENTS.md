...
- Stage 2 `sales-facts.sqlite` is governed by the approved `docs/superpowers/specs/2026-08-13-sales-facts-sqlite-design.md` and is being implemented as an independent project: daily `SID + normalized MSKU + currency mode` OrderProfit facts, monthly custom fees, effective-dated Listing owners, explicit coverage, atomic scope refresh, and revision-aware 12-hour derived caches. It remains separate from the product catalog and inventory snapshots.
