...
Operationally, the native `better-sqlite3` dependency is checked with the disposable product-catalog smoke and the full sales-facts WAL/CRUD/commit/rollback/quick-check/integrity smoke immediately after `npm ci`. Deployment then validates the sales-facts schema and, by default, an operator-supplied SHA-256-pinned approved read-only preflight artifact before the deterministic legacy product-catalog migration and PM2 restart, followed by `/api/health` and deployed-integrity checks. An operator may explicitly set `SKIP_SALES_FACTS_PREFLIGHT=1` for a deployment that must bypass only the sales-facts business preflight; this does not bypass package-manifest, branch-confirmation, CSS, SQLite, schema, migration, PM2, health, or deployed-integrity guards. The sales facts database is `data-cache/sales-facts/sales-facts-v1.sqlite`; its WAL/SHM companions remain under `data-cache/` and are excluded from deploy archives while being preserved by rollback. Legacy sales OrderProfit/weekly/monthly JSON remains read-only for reconciliation, and legacy `shared-product-catalog` and `supplier-board-product-map` JSON files remain read-only during the observation period; deleting or rewriting any of them requires a separate cleanup approval.
...
- Historical inventory-provision quantities and FIFO ages are rebuilt from an audited opening balance plus Lingxing `/cost/center/ods/detail/query`, whose events are sourced from Amazon `GET_LEDGER_DETAIL_VIEW_DATA`. Read each scope's `2024-09` official FBA monthly ending sellable inventory as the opening quantity, retain it as an explicit `OpeningSnapshot` cohort in the `271+` age bucket, then apply detail events from `2024-10`; write target history from `2025-10`, and keep costs in the separate product-management cost-refresh workflow. Never infer an opening cohort's receipt date or assign it to a younger bucket.
- `src/services/inventoryLedgerRawReportService.js` orchestrates paginated opening-snapshot/detail retrieval, strict parsing, FIFO reconstruction, and all-or-nothing historical-cache replacement. `src/services/inventoryLedgerRawReportStore.js` owns archived normalized API JSON, manifests, job state, and atomic history-directory commits. Do not add binary I/O to `src/utils/cacheStore.js`.
