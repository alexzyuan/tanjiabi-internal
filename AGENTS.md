...
10. Jiufang ShipFrom must use the documented fields `CompanyNameCn`, `CompanyNameEn`, `AttentionName`, `Phone`, and `EnterpriseCreditCode`; dry-run must fail before real create if any of these legal sender fields are missing. Store owner, not destination country, determines the legal sender: any `xiamentanjia*` store uses 厦门探嘉网络科技有限公司 / 91350206MAD64HGE0K / 厦门火炬高新区软件园三期诚毅北大街56号2302单元-3室之2D; any `tandanbo*` store uses 厦门坦蛋伯网络科技有限公司 / 91350206MADNM7UF44 / 厦门火炬高新区软件园三期诚毅北大街56号2302单元-3室之1D. Runtime `.env` values (`FBA_TANJIA_COMPANY_NAME_CN`, `FBA_TANJIA_ENTERPRISE_CREDIT_CODE`, `FBA_TANJIA_ADDRESS_LINE1`, `FBA_TANDANBO_COMPANY_NAME_CN`, `FBA_TANDANBO_ENTERPRISE_CREDIT_CODE`, `FBA_TANDANBO_ADDRESS_LINE1`, and optional city/province/postal overrides) may override these user-provided defaults; do not infer legal sender data from country or invent unknown store mappings.
11. Jiufang invoice rows must use Jiufang-documented field names such as `ShipmentId`, `ReferenceId`, `Sku`, `ProductNameCn`, `ProductNameEn`, `HsCode`, `CustomsClearanceCode`, `PurchasingPrice`, `Num`, and `MeasurementUnit`. Do not send only legacy/local names such as `ShipmentID`, `SKU`, `EnglishName`, `CustomsCode`, `UnitPrice`, or `Quantity`.
