...
- Read each scope's `2024-09` official FBA monthly ending sellable inventory as the audited opening quantity, retain it as an explicit `OpeningSnapshot` cohort in the `271+` age bucket, then apply exported ledger reports from `2024-10`; write target history from `2025-10`, and keep costs in the separate product-management cost-refresh workflow. Never infer an opening cohort's receipt date or assign it to a younger bucket.
- `src/services/inventoryLedgerRawReportService.js` orchestrates opening-snapshot retrieval, export-task lifecycle, strict report parsing, FIFO reconstruction, and all-or-nothing historical-cache replacement. `src/services/inventoryLedgerRawReportStore.js` owns archived report binaries, manifests, job state, SHA-256 verification, and atomic history-directory commits. Do not add binary I/O to `src/utils/cacheStore.js`.
