...
因此本次工作的第一性目标不是“把 JSON 搬进数据库”，而是把缓存身份从请求快照改成商品实体。最终方案是：保留现有 Node.js、PM2、领星适配器和业务服务结构，只将共享商品目录重构为基于 `better-sqlite3` 的本地实体级缓存。领星仍是最终业务数据源，SQLite 只是服务器本地的统一复用层，不扩展为订单、利润或其他业务数据数据库。
...
- 不把 OrderProfit、销售周报、店铺利润、FBA 库存、工厂库存结果等其他缓存迁移进 SQLite。
- 不在本次修改销售/订单利润的 30 分钟、6 小时或 12 小时缓存策略。
- 不安装或运行 MySQL、PostgreSQL、Redis 等独立服务。
- 不将 SQLite 作为领星之外的业务数据源，也不允许在 SQLite 中人工编辑业务字段。
...
技术栈发生变化时，实施提交必须同步更新 `AGENTS.md` 和相关部署/运行文档，记录 SQLite 文件位置、身份主键、手动刷新规则、迁移与禁止保存 raw 的约束。
...
10. 首阶段不删除旧缓存，不修改销售/订单利润缓存，不触碰无关 UI/CSS。

## 后续独立项目

商品目录稳定后，再单独设计销售与利润缓存统一：为销售周报、MSKU 明细、OrderProfit、店铺利润和经营月报定义统一的 `cacheState`、`updatedAt`、`ageSeconds`、`source` 与当前筛选范围强制刷新协议；移除无 TTL 的旧看板兜底，并解决底层 30 分钟与页面外层 6 小时不一致。本设计不提前实现这些改变。
