...
| `sales-facts.sqlite` | 按时间和币种变化的经营事实 | 自然日 + SID + MSKU + 币种模式；自定义费用按自然月 | 当前月12小时、上月24小时、更早月份冻结；显式范围强刷 | 第二阶段设计已批准，实施计划待批准 |
...
第二阶段的独立设计已于 2026-08-13 完成，完整契约见 `docs/superpowers/specs/2026-08-13-sales-facts-sqlite-design.md`。其中已经明确：
...
这些决策仍必须在独立销售事实分支中按批准计划实施；不得在商品目录维护中顺带创建销售事实表或改变 TTL。
...
2. **第二阶段设计门**：`docs/superpowers/specs/2026-08-13-sales-facts-sqlite-design.md` 已批准 `sales-facts.sqlite` schema、自动/手动刷新、负责人历史修正和对账方案；下一门槛是用户复核书面规范并批准独立实施计划。该阶段统一 `cacheState`、`updatedAt`、`ageSeconds`、`source`，移除无 TTL 的旧销售看板兜底，并解决底层 30 分钟与页面外层 6 小时不一致。
