...
2. 运行时店铺集合只来自领星 seller API 的成功响应或其缓存；静态 `lingxingShopMap` 不能作为列表或法定主体 fallback。
...
- 法定发件主体不维护逐店审批表：运行时店铺名称包含 `tanjia`（或中文展示标识“探嘉”）时归属厦门探嘉，包含 `tandanbo`（或“坦蛋伯”）时归属厦门坦蛋伯；两个品牌标识均未命中时保持失败。

## 领域规则

- 具体店铺以 `sid` 为稳定身份，`name` 为领星标识；展示名称由后端映射一次后供前端复用。
- 静态店铺映射不能在运行时 seller 数据缺失时生成可选店铺。法定主体只按运行时店铺名称中的已确认品牌标识解析，不按 SID、国家或站点推断。
...
3. `tanjia-eu-DE` 与 `tanjia-eu-UK` 只有存在于运行时店铺目录时才会出现在 FBA 物流选项中，并按 `tanjia` 品牌规则解析为厦门探嘉主体；出现未知 SID/name 或未知品牌的物流请求在 payload 生成前失败。
