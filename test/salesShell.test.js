...
test("sales shell owns front date range control bindings", () => {
  ...
  assert.deepEqual(bindCalls.map(([, selector, eventName]) => [selector, eventName]), [
    ["#front-date-range-button", "click"],
    ["#front-date-apply", "click"],
  ]);
  assert.deepEqual(bindAllCalls.map(([, selector, eventName]) => [selector, eventName]), [["[data-range-preset]", "click"]]);
  assert.deepEqual(clickOutsideCalls.map(([, selector]) => selector), [".date-range-control"]);

  bindAllCalls[0][3].call({ dataset: { rangePreset: "week" } });
  bindCalls[1][3]();
